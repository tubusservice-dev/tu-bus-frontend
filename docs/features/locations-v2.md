...
> **Status (2026-09-24):** phases 0–6 built and tested. Phases 1–3 are live in production (server + admin panel, production migrated on 2026-09-23). Phases 4–6 (customer web + checkout) are tested but **not published**; they ship together with the next mobile app release. Phase 7 (server-side shipping-cost validation) and phase 8 (legacy cleanup) are pending.
...
- Frontend: `npx tsc --noEmit -p tsconfig.app.json`, `npx ng test --watch=false --browsers=ChromeHeadless` (115 tests), and **always** `npx ng build` (templates are only type-checked by the build).

**Next steps**
1. Publish the phase-6 server changes (additive: `location` on orders/users; safe for 1.1.2).
...
5. ~~Refactors~~ — done 2026-09-24 (shared checkout contact helper; oversized files split, see §4). Remaining oversized files listed in §4.
...
7. **Store pickup placeholder removed (2026-09-24):** the summary no longer shows `settings.dispatch.storePickup` (it held example data in production: «Av. Principal #123…»), and orders no longer fall back to it — pickup always stores the selected branch's real address. The admin settings fields for it are now unused (candidate for removal).
