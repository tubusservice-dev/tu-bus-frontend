 ...
 *
 * Visual states:
 *   - `granted` + token registered → toggle ON, badge "Permitido".
 *   - `granted` + no token         → toggle OFF, click re-registers.
 *   - `default`                    → toggle OFF, click triggers prompt.
 *   - `denied`                     → toggle OFF + "¿bloqueado?" link
 *                                    showing how to unblock in the browser.
 *   - `unsupported`                → toggle disabled, "no soportado" badge.
 *
 * The native browser prompt only fires when the user clicks the toggle
 * (a real user gesture). That's why this component exists: the automatic
 * flows we tried earlier are silently dropped by Safari / Brave / Firefox.
 */
@Component({
  ...
  template: `
    <div class="ppt-wrap" [class.is-denied]="permission() === 'denied'">
      <button
        type="button"
        class="ppt-toggle"
        role="switch"
  ...
  styles: [`
    ...
    .ppt-toggle.on .ppt-thumb { transform: translateX(20px); }

    .ppt-text { display: flex; flex-direction: column; gap: 0.125rem; min-width: 0; }
    .ppt-label { font-size: 0.875rem; font-weight: 600; color: var(--text-primary, #111827); }
    :host-context(.dark) .ppt-label { color: #f3f4f6; }
    .ppt-desc { font-size: 0.75rem; color: var(--text-secondary, #6b7280); line-height: 1.3; }
